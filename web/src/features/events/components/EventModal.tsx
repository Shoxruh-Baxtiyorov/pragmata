...
import { AlertTriangle, Check, eventIcon, ShieldAlert, Users, X } from '@/shared/ui/icons'
    ...
    <Modal onClose={onClose}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2.5">
          <span
            className="flex size-10 flex-shrink-0 items-center justify-center rounded-card"
            style={{ color, background: `color-mix(in srgb, ${color} 12%, transparent)` }}
          >
            <Icon size={20} />
          </span>
          <div className="min-w-0">
            <p className="text-h3 text-text-primary">{eventLabel(event.type, lang)}</p>
            <p className="truncate text-label text-text-secondary" title={event.camera}>
              {event.camera}
      ...
      <div className="mt-4 space-y-3">
        {clip ? (
          <video src={clip} controls className="w-full rounded-card bg-black" />
        ) : photo ? (
          <img src={photo} alt="" className="w-full rounded-card" />
        ) : event.photo_url || event.clip_url ? (
          <Skeleton className="aspect-video w-full" />
        ) : (
          <div className="flex aspect-video w-full items-center justify-center rounded-card bg-bg-secondary">
            <Icon size={32} className="text-text-secondary" />
          </div>
        )}
        {event.description ? (
          <p className="rounded-card bg-bg-secondary p-3 text-body text-text-secondary">
            {event.description}
          </p>
        ) : null}
