...
class Settings(BaseSettings):
    ...
    # antelopev2 (ResNet100 glintr100) — сильнейшая модель insightface: устойчивее
    # к очкам, головным уборам, ракурсу и свету, чем buffalo_l (r50). Инференс на
    # CPU тяжелее (~250мс/лицо), но для watchlist (не каждый кадр) — ок. Смена
    # модели меняет пространство эмбеддингов → людей надо переэмбеддить по фото.
