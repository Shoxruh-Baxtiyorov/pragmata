...
from pragmata.alerts import BotSink, ClipSink
from pragmata.clips import ClipWorker, SegmentRecorder
from pragmata.config import SiteConfig, get_settings, load_site_config
...
from pragmata.sinks import ConsoleSink, DbSink, EventSink, JsonlSink, MultiSink
        ...
        *,
        ...
        if clip_cams and db_sink is not None:
            ...
            def persist_clip(event_id: uuid.UUID, path: str) -> None:
                from pragmata.db.queries import update_clip_path
        ...
        self.workers = [
            CameraWorker(
                ...
                vehicle_park_watcher=vehicle_park_watcher,
                heatmap=heatmap,
...
def main() -> None:
    parser = argparse.ArgumentParser("pragmata", description="Pragmata AI — camera pipeline")
    ...
    log = logging.getLogger("pragmata")
    ...
    if db_sink is not None:
        from pragmata.db.config_store import (
            config_version,
            load_config_from_db,
    ...
    vehicle_park_watcher = VehicleStationaryWatcher(detector)
    heatmap_acc = HeatmapAccumulator(settings.media_dir)
    # LPR ленивый: модель грузится при первом кадре камеры с включённым модулем
    lpr_reader = LprReader(device=settings.torch_device)
    ...
    if settings.telegram_bot_token and db_sink is not None and not args.no_bot:
        ...
        from pragmata.db.queries import get_clip_path, save_feedback
        from pragmata.digest import build_digest_text, seconds_until

        sf = db_sink._session_factory

        def find_person_cmd(query: str) -> list[tuple[str, str]]:
            from pragmata.investigation import find_people
    ...
    if settings.llm_api_key and settings.vlm_model and db_sink is not None:
        from pragmata.alerts import VlmSink
        from pragmata.db.queries import update_description
        from pragmata.vlm import HourBudget, VlmDescriber, VlmWorker
    ...
    try:
        while not stop_event.is_set():
            if db_sink is not None:
                from pragmata.db.config_store import config_version, load_config_from_db
