# Промт для фронтенда Soqchi AI (передать AI-ассистенту Отабека)

> Как пользоваться: скопируй ВСЁ, что ниже черты, в Claude Code / Cursor в корне репозитория
> `soqchi-ai`. Бэкенд уже работает: `uv run uvicorn soqchi.api.app:app --port 8088`,
> живая OpenAPI-спека — http://127.0.0.1:8088/docs. Вопросы по API — Шохруху.

---

Ты — senior frontend-инженер. Строишь веб-дашборд для **Soqchi AI** — системы
AI-видеонаблюдения (President AI Award 2026). Бэкенд готов и работает, его НЕ трогаем.

## Контекст продукта

Soqchi AI подключается к существующим камерам, детектит людей, события
